setInterval(function(){
    document.getElementsByClassName("rows")[0].innerHTML+=`<div class="row">
<div class="cell _capitalize _nowrap">
    <div class="icon folders-icon _dice"></div>Dice Twice
</div>
<div class="cell _user _fw500">
    <div class="inner">Thorough Swordfis...</div>
</div>
<div class="cell _time _fw500"><span>7:02 PM</span></div>
...
setInterval(function(){
    ...
    <div class="cell _user _fw500">
        <div class="inner">Old Monkey</div>
    </div>
    <div class="cell _time _fw500"><span>7:02 PM</span></div>
    <div class="cell amount _amount _fw600">$0.10</div>
    <div class="cell _fw500">0.00x</div>
...
document.getElementsByClassName("game-header__button")[1].addEventListener('click', function () {
    ...
    var balance
    var balancePrompt = prompt("Balans")
    if (isNaN(Number(balancePrompt)) == true) {
        location.reload()
    }
    else {
        balance = Number(balancePrompt)
    }
    document.getElementById("account").innerText = `₼${balance}`
    var cash = 0.15
    var multipler = 1
    ...
    document.getElementById("money").addEventListener('change', function () {
        if (Number(document.getElementById("money").value).toFixed(2) > 170) {
            cash = 170
            document.getElementById("money").value = `₼${cash.toFixed(2)}`
        }
        else if (Number(document.getElementById("money").value.slice(1)).toFixed(2) < 0.15) {
            cash = 0.15
            document.getElementById("money").value = `₼${cash.toFixed(2)}`
        }
        else if (Number(document.getElementById("money").value.slice(1)).toFixed(2) > 170) {
            cash = 170
            document.getElementById("money").value = `₼${cash.toFixed(2)}`
        }
        else {
            if (document.getElementById("money").value.includes("₼")) {
                cash = Number(document.getElementById("money").value.slice(1)).toFixed(2)
            }
            else {
                cash = Number(document.getElementById("money").value).toFixed(2)
            }
            document.getElementById("money").value = `₼${(cash * 100 / 100).toFixed(2)}`
        }
    })
    ...
    smButtons[1].addEventListener('click', function () {
        if (cash <= 0.15) {
            ...
            cash = ((Math.floor((cash - 0.1) * 100)) / 100)
        ...
        else if (cash > 1 && cash <= 10) {
            cash = ((Math.floor((cash - 1) * 100)) / 100)
            ...
            cash = ((Math.floor((cash - 10) * 100)) / 100)
    ...
    function replace() {
        ...
        if (loserButtons.length == 2) {
            ...
            document.getElementsByClassName("game-details__row-value")[0].innerText = `${25 - loserButtons.length}`
            document.getElementsByClassName("game-details__row-value")[1].innerText = `${(loserButtons.length / 25) * 100}%`
    ...
    placebuttons[0].addEventListener('click', function () {
        if (placebuttons[0].classList.contains("_placebet")) {
                ...
                buttons[i].addEventListener('click', function () {
                    if (buttons[i].classList.contains("winner")) {
                        if (!buttons[i].classList.contains("_opened")) {
                            ...
                            let count = document.getElementsByClassName("_win")
                            multipler = Number.parseFloat(document.getElementsByClassName("game-history__item-text")[count.length - 1].innerHTML.slice(1))
                            for (let i = 0; i < document.getElementsByClassName("game-history__item").length; i++) {
                                document.getElementsByClassName("game-history__item")[i].classList.remove("_active")
                            }
                            document.getElementsByClassName("game-history__item")[count.length - 1].classList.add("_active");
                            if (count.length >= 5) {
                                document.getElementsByClassName("game-history__inner-container")[0].scrollBy(78, 0)
                            }
                            ...
                            let num=Number(document.getElementsByClassName("game-details__row-value")[0].innerText)-1
                            let risk=(loserButtons.length/(Number(document.getElementsByClassName("game-details__row-value")[0].innerText)+loserButtons.length))*100
                            document.getElementsByClassName("game-details__row-value")[0].innerText = `${num}`
                            document.getElementsByClassName("game-details__row-value")[1].innerText = `${risk.toFixed(2)}%`
                            document.getElementsByClassName("game-details__row-value")[2].innerText = `${document.getElementsByClassName("_opened").length}/25`

                            


                            document.querySelector(".text-wrapper .text").innerHTML = "Cash out"
                            if (!placebuttons[1].classList.contains("_cashout")) {
                                placebuttons[1].classList.add("_cashout")
                            }
                            if (placebuttons[1].classList.contains("_placebet")) {

                                placebuttons[1].classList.remove("_placebet")
                            }
                            if (placebuttons[1].classList.contains("_cancel")) {
                                placebuttons[1].classList.remove("_cancel")
                            }
                            if (placebuttons[0].classList.contains("_placebet")) {
                                placebuttons[0].classList.remove("_placebet")
                            }
                            if (placebuttons[0].classList.contains("_cancel")) {
                                placebuttons[0].classList.remove("_cancel")

                            }
                            if (!placebuttons[0].classList.contains("_cashout")) {
            ...
            <div class="win-screen__content-payout" style="font-size: 23px;">${((Math.floor((cash * multipler * 100))) / 100).toFixed(2)}</div>
            <div class="win-screen__content-footer">
            <div class="win-screen__content-text">Çarpan</div>
            <div class="win-screen__content-odd">x${multipler}</div>
            </div>
            </div>
            </div>`
            winnedMoney = ((Math.floor((cash * multipler * 100))) / 100).toFixed(2)
            winBalance = balance + Number(winnedMoney)
            document.getElementById("account").innerText = `₼${winBalance.toFixed(2)}`
            placebuttons[0].classList.remove("_cancel")
